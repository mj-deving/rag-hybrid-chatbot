...
from src.llm_client import (
    get_client, MODEL_FAST, ROUTE_SIMPLE, ROUTE_STANDARD, ROUTE_COMPLEX, ROUTE_RELATIONAL, VALID_ROUTES,
)

CLASSIFIER_PROMPT = """Klassifiziere die folgende Frage in eine von vier Kategorien:
...
- "relational": Fragen über Beziehungen zwischen Entitäten, Verbindungen oder Netzwerke (z.B. "Wer arbeitet mit wem?", "Welche Projekte hängen zusammen?", "Wie sind X und Y verbunden?", "Welche Organisationen sind beteiligt?")

Für "complex" Fragen: Zerlege die Frage in 2-3 einfachere Sub-Queries, die jeweils separat beantwortet werden können.
Für "relational" Fragen: Extrahiere die relevanten Entitätsnamen als entity_names.

Antworte NUR mit validem JSON in diesem Format:
{"route": "simple|standard|complex|relational", "sub_queries": ["...", "..."], "entity_names": ["...", "..."]}

Bei "simple" und "standard": sub_queries und entity_names sind leere Arrays [].
Bei "complex": sub_queries enthält 2-3 zerlegte Teilfragen, entity_names ist [].
Bei "relational": entity_names enthält die relevanten Entitätsnamen, sub_queries ist []."""
...
def classify_query(question: str) -> ClassificationResult:
    ...
    try:
        ...
        entity_names = data.get("entity_names", []) if route == ROUTE_RELATIONAL else []
        return ClassificationResult(route=route, sub_queries=sub_queries, entity_names=entity_names)
